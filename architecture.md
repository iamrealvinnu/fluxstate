...
*(Handled by `EdgeVideoStream` in `fluxstate_edge/utils/video_stream.py`)*
...
## 3. Inference Engine
*(Handled by `FluxInferenceEngine` in `fluxstate_edge/core/engine.py`)*
...
*(Handled by `ForensicDatabase` in `fluxstate_edge/core/forensics.py`)*
...
*(Handled by `SemanticAgent` in `fluxstate_edge/core/agent.py`)*
...
*(Handled in the `poll_telemetry` loop in `fluxstate_edge/app.py`)*
...
*(Handled by `_push_to_integration_bus` in `fluxstate_edge/app.py`)*
